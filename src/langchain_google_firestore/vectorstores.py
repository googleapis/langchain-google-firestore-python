...
class FirestoreVectorStore(VectorStore):
    ...
    def _similarity_search(
        ...
        distance_result_field: Optional[str] = "distance"
        ...
        results = (wfilters or self.collection).find_nearest(
            ...
            limit=k,
            distance_result_field= distance_result_field if with_scores else None
        )
    ...
    def similarity_search_with_score(self, query, k = 4,filters: Optional[BaseFilter] = None, **kwargs):
        ...
        docs = self._similarity_search(
            self.embedding_service.embed_query(query), k, filters=filters,with_scores=True, distance_result_field=distance_result_field
        )
        return [
