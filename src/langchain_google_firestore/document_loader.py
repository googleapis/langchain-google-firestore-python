...
class FirestoreLoader(BaseLoader):
    def __init__(
        ...
        Args:
            source: The source to load the documents. It can be an instance of Query, CollectionGroup, DocumentReference or the single `/`-delimited path to a Firestore collection.
            page_content_fields: The document field names to write into the `page_content`. If an empty or None list is provided all fields will be written into `page_content`. When only one field is provided only the value is written.
            metadata_fields: The document field names to write into the `metadata`. By default it will write all fields that are not in `page_content` into `metadata`.
...
class FirestoreSaver:
    ...
    def __init__(
        ...
        Args:
            collection: The single `/`-delimited path to a Firestore collection. If this value is present it will write documents with an auto generated id.
            client: Client for interacting with the Google Cloud Firestore API.
        """
    ...
    def upsert_documents(
        ...
        Args:
            ...
            merge: To merge data with an existing document (creating if the document does not exist).
            document_ids: List of document ids to be used. By default it will try to construct the document paths using the `reference` from the Document.
        """
    ...
    def delete_documents(
        ...
        Args:
            documents: List of documents to be deleted from Firestore. It will try to extract the {document_path} from the `reference` in the document metadata.
            document_ids: List of documents ids to be deleted from Firestore. If provided the `documents` argument will be ignored.
