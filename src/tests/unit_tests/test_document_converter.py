...
@pytest.mark.parametrize(
    "langchain_doc,firestore_doc", [
        ...
        (Document(
         ...
         {'path':None,'data':{'page_content':'value', 'reference': {'path':'foo/bar','unexpected_field':'data'}}}),
        (Document(
            page_content='value',
            metadata={'reference':{'path':'foo/bar'},'metadata_field':{'path':'abc/xyz'}}),
         {'path':'foo/bar','data':{'page_content':'value', 'metadata_field': DocumentReference(*['abc','xyz'], client=pytest.client)}}),
        (Document(
            page_content='{"field_1": "val_1", "field_2": "val_2"}',
            metadata={'reference':{'path':'foo/bar'}, 'field_3': 'val_3'}),
         {'path':'foo/bar','data':{'field_1':'val_1','field_2':'val_2','field_3':'val_3'}}),
        (Document(
            page_content='',
            metadata={'reference':{'path':'foo/bar'}}),
         {'path':'foo/bar','data':{}}),
        (Document(
            page_content='',
            metadata={'reference':{'path':'foo/bar'},'point':{'latitude':1, 'longitude':2},'field_2':'val_2'}),
        ...
        (Document(
            ...
            metadata={}),
         {'path':None,'data':{'array':[1, 'data', {'k_1':'v_1', 'k_point': GeoPoint(1,0)}], 'f_2':2}}),
    ])
def test_convert_langchain_document(langchain_doc, firestore_doc):
  return_doc = DocumentConverter.convertLangChainDocument(langchain_doc, pytest.client)
...
@pytest.mark.parametrize(
    "firestore_doc", [
        (DocumentSnapshot(
            ...
            data={'field_1':GeoPoint(1,2),
                  'field_2':['data',2,{'nested':DocumentReference(*['abc','xyz'], client=pytest.client)}]},
            exists=True,
            read_time=None,
            create_time=None,
            update_time=None)),
...
def test_roundtrip_firestore(firestore_doc):
  ...
  roundtrip_doc = DocumentConverter.convertLangChainDocument(langchain_doc, pytest.client)
