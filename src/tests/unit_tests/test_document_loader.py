  ...
  for i in range(NUM_DOCS):
    ...
    expected_docs.append(
        Document(page_content=f"content {i}",
                 metadata={'reference':{'path':mock.ANY}}))
...
def test_firestore_write_with_reference() -> None:
  ...
  expected_doc = [Document(page_content="{'f1': 1, 'f2': 2}",
...
def test_firestore_load_with_fields(
  ...
  doc_to_insert = [Document(
      page_content="{'f1': 'v1', 'f2': 'v2', 'f3': 'v3'}",
      metadata={})]
